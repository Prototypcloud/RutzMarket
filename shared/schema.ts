...
export const globalIndigenousPlants = pgTable("global_indigenous_plants", {
  ...
  // Enhanced fields from new JSON schema
  indigenousNames: jsonb("indigenous_names").$type<string[]>().default(sql`'[]'::jsonb`),
  habitat: jsonb("habitat").$type<string[]>().default(sql`'[]'::jsonb`),
  useCategory: jsonb("use_category").$type<string[]>().default(sql`'[]'::jsonb`),
  productOpportunityType: jsonb("product_opportunity_type").$type<string[]>().default(sql`'[]'::jsonb`),
  novelFoodRegulatoryFlag: text("novel_food_regulatory_flag"),
  nutraceuticalPotential: text("nutraceutical_potential"),
  harvestCultivationStatus: text("harvest_cultivation_status"),
  timeToMarket: text("time_to_market"),
  culturalConsent: text("cultural_consent"),
  restorationServiceRole: text("restoration_service_role"),
  valueChainComplexity: text("value_chain_complexity"),
  partsUsed: jsonb("parts_used").$type<string[]>().default(sql`'[]'::jsonb`),
  seasonalityHarvestWindow: text("seasonality_harvest_window"),
  seasonalityNotes: text("seasonality_notes"),
  respectfulHarvestProtocol: text("respectful_harvest_protocol"),
  regulatoryNotes: text("regulatory_notes"),
  geofences: jsonb("geofences").$type<any[]>().default(sql`'[]'::jsonb`),
  skuIdeas: jsonb("sku_ideas").$type<string[]>().default(sql`'[]'::jsonb`),
  processingNotes: text("processing_notes"),
  populationStatus: text("population_status"),
  harvestRotationYears: integer("harvest_rotation_years").default(0),
  replantingRecommended: boolean("replanting_recommended").default(false),
  images: jsonb("images").$type<string[]>().default(sql`'[]'::jsonb`),
  sources: jsonb("sources").$type<any[]>().default(sql`'[]'::jsonb`),
  knowledgeKeeperReview: boolean("knowledge_keeper_review").default(false),
